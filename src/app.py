...
def check_login():
    ...
    with st.form("login_form"):
        top_col1, top_col2, top_col3, top_col4 = st.columns([4, 2, 2, 1])
...
# IMPORTANT:
# Login check must happen before the dashboard UI
if not check_login():
    st.stop()
...
st.caption(
    "Simulates random sensor readings every 2 seconds for 20 cycles, "
    "as if streaming from a real machine."
)
...
uploaded_file = st.file_uploader(
    ...
    type=["csv"],
    help=(
        "CSV should contain Type, Air temperature [K], Process temperature [K], "
...
if uploaded_file is not None:
    try:
        ...
        results_df = batch_df.copy()
        results_df["failure_probability"] = probs
        results_df["failure_probability_percent"] = results_df[
            "failure_probability"
        ...
        col3.metric(
            ...
            f"{results_df['failure_probability'].mean():.1%}"
        )
